...
\begin{definition}
	Let $i:X\hookrightarrow Y$ be an injective continuous map of topological spaces. Suppose that the topology on $X$ is induced by $i$. Then $i$ is \textit{an immersion of topological spaces}.
\end{definition}
...
\begin{proposition}\label{proposition:regular_monomorphisms_are_immersions}
	...
	\begin{enumerate}[label=\emph{\textbf{(\roman*)}}, leftmargin=3.0em]
		...
		\item $f$ is an immersion of topological spaces.
	\end{enumerate}
\end{proposition}
\begin{proof}
	...
	On the other hand Theorem \ref{theorem:topology_induced_on_domain_of_a_family_of_maps} asserts that $f$ is an immersion of topological spaces if and only if $f$ is a kernel of $(u_1,u_2)$. Therefore, $\textbf{(ii)}\Leftrightarrow \textbf{(iii)}$ and this completes the proof.
\end{proof}
...
\begin{fact}\label{fact:open_injective_maps_are_immersions}
	Let $i:X \hookrightarrow Y$ be an injective and open map of topological spaces. Then $i$ is an immersion.
\end{fact}
\begin{proof}
...
\begin{fact}\label{fact:closed_injective_maps_are_immersions}
	Let $i:X \hookrightarrow Y$ be an injective and closed map of topological spaces. Then $i$ is an immersion.
\end{fact}
\begin{proof}
