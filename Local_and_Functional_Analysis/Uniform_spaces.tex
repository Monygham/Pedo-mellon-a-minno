...
\noindent
These notes are devoted to uniform spaces. Uniform spaces are important objects designed to formalize notion of uniformly continuous map. The first section introduces uniform spaces and uniform maps. We discuss their category and introduce the notion of uniform strucutre induced by family of maps. In the next section we prove uniform version of Urysohn lemma and as a consequence obtain that each uniform structure is induced by some family of pseudometrics. Then we explain how each uniform structure produces completely regular topology. In the following section we introduce important classes of Hausdorff and complete uniform spaces. Then we discuss pseudometric and metric spaces, which are the most important and basic examples of uniform spaces. We also prove that each uniform space can be embedded as a dense subsapce into a complete uniform space.
...
\begin{fact}\label{fact:unifrom_structures_induced_by_families_of_pseudometrics}
...
\section{Complete pseudometric spaces and completion}
...
\end{theorem}
\begin{proof}
	Theorem \ref{theorem:each_uniform_structure_is_induced_by_pseudometrics} asserts that there exists a family $\Phi$ of pseudometrics on $X$ which induce its uniform structure. For each $\rho \in \Phi$ let $X_{\rho}$ be pseudometric space having the same underlying set as $X$ together with $\rho$ as its pseudometric. By Theorem \ref{theorem:completion_for_pseudometric_spaces} there exists a complete pseudometric space $\tilde{X}_{\rho}$ and an injective isometry $i_{\rho}:X_{\rho} \hookrightarrow \tilde{X}_{\rho}$ with dense image. We consider $i_{\rho}$ as a uniform map $X \hookrightarrow \tilde{X}_{\rho}$. Then $\{i_{\rho}\}_{\rho\in \Phi}$ is a family of maps that induce on the underlying set of $X$ precisely the uniform structure induced by the family $\Phi$. Hence $\{i_{\rho}\}_{\rho \in \Phi}$ induce the original unifomity of $X$. It follows that the uniform map
	$$\langle i_{\rho} \rangle_{\rho \in \Phi}:X\hookrightarrow \prod_{\rho \in \Phi}\tilde{X}_{\rho}$$
	is an embedding of uniform spaces. Since $\prod_{\rho \in \Phi}\tilde{X}_{\rho}$ is complete by Theorem \ref{theorem:completeness_for_pseudometric_spaces_is_equivalent_to_convergence_of_Cauchy_sequences} and Theorem \ref{theorem:product_of_complete_spaces_is_complete}, we derive that $X$ can be embedded into a complete uniform space. Next Theorem \ref{theorem:completeness_is_inheritied_by_closed_subspaces} imply that $X$ can be embedded into a complete uniform space as a dense subspace.
\end{proof}
