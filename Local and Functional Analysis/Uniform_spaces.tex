...
$$\forall_{x,y\in X}\,(x,y)\in V\,\Leftrightarrow\,(y,x)\in V$$
and $\Delta_X\subseteq V$ is \textit{a surrounding of the diagonal in $X\times X$}.    
\end{definition}

\begin{definition}
Let $X$ be a set and let $V,W$ be subsets of $X\times X$. We define a subset $W\cdot V$ of $X\times X$ called \textit{a composition of $V$ with $W$} by formula
...
\begin{theorem}\label{proposition:universal_Hausdorff_quotient}
Let $(X,\fU)$ be a uniform space. The following assertions hold.
\begin{enumerate}[label=\emph{\textbf{(\arabic*)}}, leftmargin=*]
\item The relation
$$\Delta_{\fU} = \bigcap_{U\in \fU}U \subseteq X\times X$$
...
In order to prove that $\fU_{sep}$ is a uniform structure it suffices to check that for every $U$ in $\fU_{sep}$ there exists $W \in \fU_{sep}$ such that $W\cdot W \subseteq U$. For this pick $U$ in $\fU_{sep}$. Then $(q\times q)^{-1}(U) \in \fU$. Since $\fU$ is a uniform structure, there exists a sequence $\{W_n\}_{n\in \NN_+}$ of elements of $\fU$ such that 
$$W_n \cdot ...\cdot W_2 \cdot W_1\cdot W_2 \cdot ... \cdot W_n \subseteq (q\times q)^{-1}(U)$$
for every $n\in \NN_+$. We define
$$\tilde{W} = \bigcup_{n\in \NN_+}W_n \cdot ...\cdot W_2 \cdot W_1\cdot W_2 \cdot ... \cdot W_n$$
Then $\tilde{W} \in \fU$ and $\tilde{W} \subseteq (q\times q)^{-1}(U)$. Moreover, we have
$$\Delta_{\fU}\cdot \tilde{W} \cdot \Delta_{\fU} \subseteq \tilde{W}$$
From this inclusion it follows that $W = \left(q\times q\right)(\tilde{W})$ satisfies $(q\times q)^{-1}(W) = \tilde{W}$
\end{proof}
