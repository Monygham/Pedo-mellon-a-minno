...
% \begin{theorem}\label{theorem:existence_of_completion_for_Hausdorff}
% Let $\left(X,\fU\right)$ be a Hausdorff uniform space. Then there exists a complete Hausdorff uniform space $\left(\ol{X},\ol{\fU}\right)$ and an embedding $j:\left(X,\fU\right)\hookrightarrow \left(\ol{X},\ol{\fU}\right)$ of uniform spaces such that $j(X)$ is dense in $\ol{X}$ with respect to topology induced by $\ol{\fU}$.
% \end{theorem}
