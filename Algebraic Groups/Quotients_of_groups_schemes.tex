...
Let $X$ be an affine $k$-scheme and let $\bd{G}$ be a geometrically reductive group. Suppose that $Y = \Spec \Gamma\left(X,\cO_X\right)^{\bd{G}}$ and $q:X\ra Y$ is the canonical morphism. Then $q$ is a uniform good categorical quotient of $X$. Moreover, the following assertions hold. 
\begin{enumerate}[label=\emph{\textbf{(\arabic*)}}, leftmargin=3.0em]
...
Let $\bd{G}$ be an algebraic group $\bd{G}$ which acts on $\Spec A$ for some $k$-algebra $A$. Fix a flat $A^{\bd{G}}$-algebra $B$. Then the canonical morphism $B \ra \left(A\otimes_{A^{\bd{G}}}B\right)^{\bd{G}}$ is an isomorphism of $k$-algebras.
\end{lemma}
\begin{proof}[Proof of the lemma]
