...
{ \bd{G}\times_k\Spec A &  X\times_k\Spec A                         \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
...
\item \textit{$x$ is stable with respect to $\tau$} if there exists a $\bd{G}$-invariant section $s\in \Gamma(X,\cL^{\otimes n})$ with respect to $\tau^{\otimes n}$ for some $n$ such that $X_s$ is affine, contains $x$, the action of $\bd{G}$ on $X_s$ is closed and has finite stabilizers.
\end{enumerate}
We also denote by
