...
\end{theorem}
\noindent
For the proof we need to following.

\begin{lemma}\label{lemma:flat_base_change_for_geometrically_reductive_group_action_on_affine}
Let $\bd{G}$ be an algebraic group $\bd{G}$ which acts on $\Spec A$ for some $k$-algebra $A$. Fix flat $A^{\bd{G}}$-algebra $B$. Then we have canonical isomorphism
