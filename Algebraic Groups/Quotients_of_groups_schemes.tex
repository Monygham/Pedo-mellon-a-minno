...
\begin{lemma}\label{lemma:flat_base_change_for_geometrically_reductive_group_action_on_affine}
...
\end{tikzpicture}
\end{center}
where $c:V\ra k[\bd{G}]\otimes_kV$ is the coaction. Now we denote by $c$ the coaction on $A$. Thus we have left exact sequences
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
...
(m-1-1) edge node[above] {$  $} (m-1-2)
(m-1-2) edge node[above] {$  $} (m-1-3)
(m-1-3) edge node[above] {$x\otimes 1\mapsto c(x)\otimes 1 - 1\otimes x\otimes 1  $} (m-1-6);
\end{tikzpicture}
\end{center}
and
\begin{center}
...
\end{tikzpicture}
\end{center}
Note that $A\otimes_{A^{\bd{G}}}B\ni x\otimes 1\mapsto c(x)\otimes 1\in k[\bd{G}]\otimes_kA\otimes_{A^{\bd{G}}}B$ is the coaction induced by $c$ on the base change $A\otimes_{A^{\bd{G}}}B$. This implies that there is canonical isomorphism
$$B =  A^{\bd{G}}\otimes_{A^{\bd{G}}}B \cong \left(A\otimes_{A^{\bd{G}}}B\right)^{\bd{G}}$$
\end{proof}

\begin{lemma}\label{lemma:surjectivity_of_affine_quotient}
...
\end{lemma}
\begin{proof}[Proof of the lemma]
Suppose that there exist $f_1,...,f_n\in A^{\bd{G}}$ such that the ideal generated by $f_1,...,f_n$ in $A^{\bd{G}}$ is proper but the ideal generated by these elements in $A$ is nonproper. Fix $f_1,...,f_n$ as above but with minimal possible $n$.
\end{proof}


\begin{proof}[Proof of the theorem]
Since $X$ is quasi-compact, we may verify conditions of Proposition \ref{proposition:finite_intersection_property_for_quasi_compact_suffices_for_good_categorical_quotients}. First let us denote by $A$ the $k$-algebra of global regular functions $\Gamma(X,\cO_X)$. Suppose that $V \subseteq \Spec A^{\bd{G}} = Y$ is an open affine subset. Then $B = \Gamma(V,\cO_Y)$ is a flat $A^{\bd{G}}$-algebra and by Lemma \ref{lemma:flat_base_change_for_geometrically_reductive_group_action_on_affine} we have canonical isomorphism
$$B \cong \left(A\otimes_{A^{\bd{G}}}B\right)^{\bd{G}}$$
This implies that $\Gamma(V,\cO_Y) \cong \Gamma(q^{-1}(V),\cO_X)^{\bd{G}}$ and hence $\cO_Y$ is the sheaf of $\bd{G}$-invariants for $q$. Fix now a closed $\bd{G}$-stable subset $Z$ of $X$. By {\cite[Corollary 5.4]{Algebraic_groups}} there exists a $\bd{G}$-stable ideal $\ideal{a}\subseteq A$ such that its vanishing set is equal to $Z$. Consider a commutative square
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
...
\end{tikzpicture}
\end{center}
with canonically defined arrows. Note that $\Spec A/\ideal{a} \ra \Spec (A/\ideal{a})^{\bd{G}}$ is surjective and according to {\cite[Theorem 2.4]{Geometrically_reductive_and_Nagata}} morphism
