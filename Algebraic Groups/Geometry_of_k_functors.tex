...
\section{Open $k$-subfunctors and criterion for representability}
\noindent
...
Let $\sigma:\fX\ra \fY$ be a morphism of Zariski local $k$-functors. Then the following assertions are equivalent.
\begin{enumerate}[label=\emph{\textbf{(\roman*)}}, leftmargin=3.0em]
...
\begin{enumerate}[label=\emph{\textbf{(\roman*)}}, leftmargin=3.0em]
\item For every $k$-algebra $A$ and every morphism $\tau:\Spec A \ra \fY$ of $k$-functors consider a cartesian square
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
...
\end{tikzpicture}
\end{center}
where $X$ is a $k$-scheme. Then $f$ as a morphism of $k$-schemes is in $\cC$.
\item For every $k$-scheme $Y$ and every morphism $\tau:Y\ra \fY$ of $k$-functors consider a cartesian square
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
...
\begin{remark}\label{remark:various_classes_of_morphisms}
...
\begin{proposition}\label{proposition:open_closed_immersions_for_arbitrary_k_functors}
...
Let $\fY$ be a Zariski local $k$-functor such that the diagonal $\fY\ra \fY\times \fY$ is representable. Then every morphism $\sigma:\fX\ra \fY$ of Zariski local $k$-functors is representable.
\end{proposition}
\begin{proof}
Fix a morphism of Zariski local $k$-functors $\sigma:\fX\ra \fY$. Let $Y$ be a $k$-scheme and let $\tau:Y\ra \fY$ be a morphism of $k$-functors. Consider the cartesian square
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
...
\end{tikzpicture}
\end{center}
Since the diagonal of $\fY$ is representable, we derive that $\fZ$ is isomorphic with a $k$-scheme. This finishes the proof.
\end{proof}

\section{Remarks on Zariski sheaves on $\Sch_k$}
...
{  \fX                       & \mathrm{Grass}_{V,n}           \\
   \Spec A            & \mathrm{Grass}_{V,n}\times \mathrm{Grass}_{V,n}           \\} ;
\path[->,line width=1.0pt,font=\scriptsize]
(m-1-1) edge node[above] {$  $} (m-1-2)
(m-2-1) edge node[below] {$ \tau $} (m-2-2);
\path[right hook->,line width=1.0pt,font=\scriptsize]
...
\end{tikzpicture}
\end{center}
In the diagram $pr^A_i$ is the projection on $i$-th component. Diagram implies that $\left(pr^B_i\cdot f^{\oplus I}\right)\left(\ideal{J}\right)=0$ for every $i\in I$ if and only if $\left(f\cdot pr_i^A\right)(\ideal{J}) = 0$ for every $i\in I$. This is equivalent with the condition that $f(\ideal{a})=0$ for ideal $\ideal{a}$ in $A$ generated by $\sum_{i\in I}pr_i^A(\ideal{J})$. Thus the lemma is proved.
\end{proof}
...
{  \fU  & \iMor_k\left(X,\fY'\right) \\
   \Spec A & \iMor_k\left(X,\fY\right)           \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[above] {$ $} (m-1-2)
(m-2-1) edge node[below] {$ \sigma $} (m-2-2)
(m-1-1) edge node[left] {$  $} (m-2-1)
(m-1-2) edge node[right] {$ \iMor_k\left(1_{X},j\right) $} (m-2-2);
\end{tikzpicture}
\end{center}
