...
(m-1-2) edge node[above] {$ r\mapsto r_{\mid q^{-1}(V_i)} $} (m-1-3);
\end{tikzpicture}
\end{center}
...
\section{Rudimentary results on line $\bd{G}$-bundles}
\noindent
...
{ a^*\cL_{\mid \bd{G}\times_kX_s}  &     &  \mathrm{pr}_X^*\cL_{\mid \bd{G}\times_kX_s}  \\
                    &\cO_{\bd{G}\times_k X_s}  &  \\} ;
\path[->,line width=1.0pt,font=\scriptsize]
(m-1-1) edge node[above] {$ \tau^{-1}_{\mid \bd{G}\times_k X_s} $} (m-1-3)
...
\section{Application: $\mathbb{G}_m$-quotients and projective bundles}
\noindent
...
(m-1-1) edge node[left] {$1_{\bd{G}}\times_kq $} (m-2-1)
(m-1-2) edge node[auto] {$ q $} (m-2-2);
\path[densely dotted,->,line width=1.0pt,font=\scriptsize]  
(m-2-1) edge node[below] {$\tilde{a} $} (m-2-2);
\end{tikzpicture}
\end{center}
...
\begin{proposition}\label{proposition:commuting_actions_induce_equivariant_quotient_of_morphism}
...
(m-1-2) edge node[auto] {$ q_2 $} (m-2-2)
(m-2-1) edge node[below] {$g $} (m-2-2);
\end{tikzpicture}
\end{center}
...
\begin{theorem}\label{theorem:actions_of_vector_bundles_with_linearization}
...
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=2em, column sep=7em,text height=1.5ex, text depth=0.25ex] 
{ \bd{G}\times_k\mathbb{V}\left(\cE\right) = \Spec_{\bd{G}\times_kX}\,\Sym\left(\mathrm{pr}_X^*\cE\right) & \Spec_{\bd{G}\times_kX}\,\Sym\left(a^*\cE\right)  = \mathbb{V}\left(a^*\cE\right)   \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[auto] {$  \Spec_{\bd{G}\times_kX}\,\Sym\left(\tau^{-1}\right) $} (m-1-2);
\end{tikzpicture}
\end{center}
...
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
{   \mathbb{V}\left(a^*\cE\right)                 &    \mathbb{V}(\cE)                           \\
    \bd{G}\times_kX &   X                 \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[auto] {$a' $} (m-1-2)
...
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
{   \bd{G}\times_k\big(\mathbb{V}\left(\cE\right)\setminus Z\big)& \mathbb{V}\left(\cE\right)\setminus Z \\
    \bd{G}\times_kX &   X                 \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[auto] {$ $} (m-1-2)
(m-2-1) edge node[below] {$ a$} (m-2-2)
...
\begin{example}\label{example:projective_bundles_as_quotients}
Let $X$ be a $k$-scheme and let $\cE$ be a quasi-coherent sheaf on $X$. Consider 
$$\mathbb{V}(\cE) = \Spec_X\,\Sym\left(\cE\right)$$
...
\begin{corollary}\label{corollary:actions_on_projective_bundles}
...
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
{   \bd{G}\times_k\big(\mathbb{V}\left(\cE\right)\setminus Z\big)& \mathbb{V}\left(\cE\right)\setminus Z \\
    \bd{G}\times_k\mathbb{P}(\cE) & \mathbb{P}(\cE)\\
    \bd{G}\times_kX &   X                 \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[auto] {$ \tilde{a}$} (m-1-2)
(m-3-1) edge node[below] {$ a$} (m-3-2)
(m-1-1) edge node[left] {$1_{\bd{G}}\times_k \pi $} (m-2-1)
(m-1-2) edge node[auto] {$ \pi $} (m-2-2)
(m-2-1) edge node[left] {$ 1_{\bd{G}}\times_kp $} (m-3-1)
(m-2-2) edge node[auto] {$ p $} (m-3-2)
(m-2-1) edge node[auto] {$ \ol{a} $} (m-2-2);
\end{tikzpicture}
\end{center}
...
\begin{corollary}\label{corollary:universal_property_of_projective_bundles_and_equivariance}
...
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=4em, column sep=2.5em,text height=1.5ex, text depth=0.25ex] 
{ \mathbb{V}(f^*\cE)\setminus f^{-1}(Z) &  \mathbb{V}(\cE)\setminus Z\\
  X                                     &  Y                         \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[auto] {$ f'$} (m-1-2)
...
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=4em, column sep=2.5em,text height=1.5ex, text depth=0.25ex] 
{ \mathbb{V}(\cL)\setminus S &   & \mathbb{V}(f^*\cE)\setminus f^{-1}(Z) \\
                             & X &                                                           \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[auto] {$ \theta $} (m-1-3)
(m-1-1) edge node[left = 2pt, below = 1pt] {$ p $} (m-2-2)
(m-1-3) edge node[right = 2pt, below = 1pt] {$ q' $} (m-2-2);
\end{tikzpicture}
\end{center}
...
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=4em, column sep=2.5em,text height=1.5ex, text depth=0.25ex] 
{ \mathbb{V}(\cL)\setminus S &  \mathbb{V}(\cE)\setminus Z\\
    X                                     &  \mathbb{P}(\cE)                         \\} ;
\path[->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[auto] {$ f'\cdot \theta$} (m-1-2)
(m-2-1) edge node[below] {$ g $} (m-2-2)
...
\begin{theorem}\label{theorem:quotient_determined_by_linearization}
