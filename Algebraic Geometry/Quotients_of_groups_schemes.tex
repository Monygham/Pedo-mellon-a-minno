...
Suppose that $\bd{G}$ is a smooth locally algebraic group over $k$. Let $q:X\ra Y$ be a geometric quotient and assume that $Y$ is the spectrum of a complete local noetherian $k$-algebra such that the residue field of the closed point of $Y$ is $k$. Then the following assertions hold.
...
\end{theorem}
\noindent
It is usefull to extract some parts of the argument into lemmas.
...
Let $q:X\ra Y$ be a geometric quotient and assume that $Y$ is the spectrum of a local $k$-algebra such that the residue field of the closed point $o$ of $Y$ is $k$. Let $x$ be the $k$-point of $X$ with orbit morphism that is a closed immersion, then $q^{-1}(o)$ is the closed subscheme of $X$ determined by the orbit morphism $\bd{G}\hookrightarrow X$ of $x$. 
\end{lemma}
\begin{proof}[Proof of the lemma]
Denote by $\bd{G}x$ the closed subscheme determined by the orbit morpism $\bd{G}\hookrightarrow X$ of $x$. Morphism $q$ induces the morphism of residue fields $k(q(x)) \hookrightarrow k(x) = k$ over $k$. This implies that $k(q(x)) = k$ and hence $q(x)$ is a $k$-point of $Y$. Note that $o$ is the unique $k$-point of $Y$. Thus $x\in q^{-1}(o)$. Clearly $q^{-1}(o)$ is a closed $\bd{G}$-stable subscheme of $X$ (it is the preimage of $o$ under the $\bd{G}$-equivariant morphism), that contains $x$.  Since $\bd{G}x$ is the smallest closed $\bd{G}$-stable subscheme of $X$ containing $x$, we deduce that $\bd{G}x\subseteq q^{-1}(o)$. Recall that we have a cokernel diagram 
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
