...
\end{fact}
\noindent
The following result states that over special type of local complete noetherian $k$-algebras free actions correspond to trivial $\bd{G}$-bundles. 
...
\begin{definition}
Let $q:X\ra Y$ be a $\bd{G}$-equivariant morphism into a $k$-scheme $Y$ equipped with the trivial $\bd{G}$-action. Suppose that $q$ is faithfuly flat and the square
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
...
X &   Y                 \\} ;
\path[right hook->,line width=1.0pt,font=\scriptsize]  
(m-1-1) edge node[auto] {$ \mathrm{pr}_X $} (m-1-2)
(m-2-1) edge node[below] {$ q $} (m-2-2)
(m-1-1) edge node[left] {$ q $} (m-2-1)
(m-1-2) edge node[auto] {$ \mathrm{pr}_X$} (m-2-2);
\end{tikzpicture}
\end{center}
...
Suppose that $\bd{G}$ is a smooth algebraic group over $k$. Let $q:X\ra Y$ be a morphism locally of finite type between $k$-schemes locally of finite type. Then the following assertions are equivalent.
\begin{enumerate}[label=\emph{\textbf{(\roman*)}}, leftmargin=3.0em]
