...
\end{definition}
\noindent
If $X$ is a set and $A$ is its subset, then we denote by $\mathbb{1}_A$ the function on $X$ defined by $\mathbb{1}_A(x)=1$ for $x\in A$ and $\mathbb{1}_A(x)=0$ otherwise. The codomain of $\mathbb{1}_A$ is either $\ol{\RR}$ or some field $\mathbb{K}$ equipped with an absolute value. In either case the codomain can be clearly deduced from the context.
...
\end{fact}
\begin{proof}
Let $N$ be a set in $\Sigma$ such that $\mu(N) = 0$ and $f(x)\leq g(x)$ for $x\not \in N$. Consider a nonnegative, simple function $s:X\ra \ol{\RR}$ such that $s\leq f$. Then $\mathbb{1}_{X\setminus N}\cdot s:X\ra \ol{\RR}$ is also a nonnegative, simple function. Moreover, we have
$$\int_X\mathbb{1}_{X\setminus N}\cdot s\,d\mu = \int_Xs\,d\mu$$
and $\mathbb{1}_{X\setminus N}\cdot s \leq g$. Thus
$$\int_Xs\,d\mu = \int_X\mathbb{1}_{X\setminus N}\cdot s\,d\mu \leq \int_Xg\,d\mu$$
Hence also
$$\int_Xf\,d\mu \leq \int_Xg\,d\mu$$
...
$$\alpha  \int_Xs\,d\mu = \int_X\alpha s\,d\mu = \int_X \alpha \mathbb{1}_{X\setminus \left(A_n\cup N\right)}\cdot s\,d\mu +  \int_X \alpha \mathbb{1}_{A_n\cup N}\cdot s\,d\mu \leq $$
...
$$||f||_1 = \int_X||f||\,d\mu = \int_X\mathbb{1}_{X\setminus Z}\cdot \big|\big|f_{n_0} + \sum_{k\in \NN}(f_{n_{k+1}} - f_{n_k})\big|\big|\,d\mu \leq \int_X\left(||f_{n_0}|| + \sum_{k\in \NN}||f_{n_{k+1}}-f_{n_k}||\right)\,d\mu = $$
...
$$||f - f_{n_m}||_1 = \int_X\mathbb{1}_{X\setminus Z}\cdot \big|\big|\sum_{k\geq m}\left(f_{n_{k+1}} - f_{n_k}\right)\big|\big|\,d\mu \leq  \int_X \big|\big|\sum_{k\geq m}\left(f_{n_{k+1}} - f_{n_k}\right)\big|\big|\,d\mu \leq $$
...
$$\int_X2g\,d\mu - \int_X\limsup_{n\ra +\infty}||f-f_n||\,d\mu  = \int_X\mathbb{1}_{X\setminus N}\cdot 2g\,d\mu - \int_X\mathbb{1}_{X\setminus N}\cdot \limsup_{n\ra +\infty}||f-f_n||\,d\mu = $$
$$= \int_X\liminf_{n\ra +\infty}\mathbb{1}_{X\setminus N}\cdot  \left(2g - ||f-f_n||\right)\,d\mu \leq \liminf_{n\ra +\infty}\int_X\mathbb{1}_{X\setminus N}\cdot \left(2g - ||f- f_n||\right)\,d\mu =$$
$$= \int_X\mathbb{1}_{X\setminus N}\cdot 2g\,d\mu - \limsup_{n\ra +\infty}\int_X\mathbb{1}_{X\setminus N}\cdot||f-f_n||\,d\mu = \int_X2g\,d\mu - \limsup_{n\ra +\infty}\int_X||f-f_n||\,d\mu$$
Hence
...
This function is well defined according to Proposition \ref{proposition:measurable_functions_have_measurable_sections}. Let $\cF$ be a family of all subsets $E\in \Sigma_1\otimes \Sigma_2$ such that $f_E$ is measurable with respect to $\Sigma_1$. First note that if $E=A\times B$ for $A\in \Sigma_1$ and $B\in \Sigma_2$, then  $f_E = \mu_2(B)\cdot \mathbb{1}_A$. Now suppose that
$$E= \bigcup_{n=1}^mA_n\times B_n$$
where $A_n\in \Sigma_1,B_n\in \Sigma_2$ for every $1\leq n\leq m$. Then
$$f_E = \sum_{n=1}^m\mu_2(B_n)\mathbb{1}_{A_n}$$
...
$$(\mu_1\otimes \mu_2)\left(A\times B\right) = \int_{X_1}\mu_2(B)\mathbb{1}_A\,d\mu_1 = \mu_1(A)\mu_2(B)$$
...
\end{theorem}
\begin{proof}
Let $N$ be a set in $\Sigma$ such that $\left(\mu_1\otimes \mu_2\right)(N) = 0$ and $f_{\mid X_1\times X_2\setminus N}$ is measurable with respect to $\Sigma_1\otimes \Sigma_2$. We write $g = \mathbb{1}_{X_1\times X_2\setminus N}\cdot f$ and $r = \mathbb{1}_N\cdot f$. Then $g$ is measurable with respect to $\Sigma_1\otimes \Sigma_2$. According to Theorem \ref{theorem:fubinis_theorem_basic} we derive that
$$\int_{X_1}\mu_2(N_{x_1})\,d\mu_2 = \int_{X_1\times X_2}\mathbb{1}_N\,d\left(\mu_1\otimes \mu_2\right) = \int_{X_2}\mu_1(N_{x_2})\,d\mu_1$$
and hence
...
$$\int_{X_1}\int_{X_2} \left(\mathbb{1}_{\left(X_1\setminus N_1\right)\times X_2}\right)_{x_1}\cdot f_{x_1}\,d\mu_2d\mu_1 = \int_{X_1} \int_{X_2} \left(\mathbb{1}_{\left(X_1\setminus N_1\right)\times X_2}\right)_{x_1}\cdot  g_{x_1}\,d\mu_2d\mu_1 =$$
$$= \int_{X_1\times X_2}\mathbb{1}_{\left(X_1\setminus N_1\right)\times X_2}\cdot g\,d\left(\mu_1\otimes \mu_2\right) = \int_{X_1\times X_2}g\,d\left(\mu_1\otimes \mu_2\right) = \int_{X_1\times X_2}f\,d\left(\mu_1\otimes \mu_2\right)$$
and
$$\int_{X_2} \int_{X_1} \left(\mathbb{1}_{X_1\times \left(X_2\setminus N_2\right)}\right)_{x_2}\cdot f_{x_2}\,d\mu_1d\mu_2 = \int_{X_2} \int_{X_1} \left(\mathbb{1}_{X_1\times \left(X_2\setminus N_2\right)}\right)_{x_2}\cdot g_{x_2}\,d\mu_1d\mu_2 =$$
$$= \int_{X_1\times X_2}\mathbb{1}_{X_1\times \left(X_2 \setminus N_2\right)}\cdot g\,d\left(\mu_1\otimes \mu_2\right) = \int_{X_1\times X_2}g\,d\left(\mu_1\otimes \mu_2\right) = \int_{X_1\times X_2}f\,d\left(\mu_1\otimes \mu_2\right)$$
\end{proof}
