...
\end{tikzpicture}
\end{center}
Finally note that for every $k$-scheme $X$ we have relative affine spectrum functor $\Spec_X:\Alg\left(\Qcoh(X)\right)\ra \Sch_X$. Moreover, if $f:X\ra Y$ is a morphism of $k$-schemes and $\cA$ is a quasi-coherent $\cO_Y$-algebra, then the canonical square
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=2em, column sep=2em,text height=1.5ex, text depth=0.25ex] 
...
\end{tikzpicture}
\end{center}
is cartesian. Thus the collection of all these functors for all $k$-scheme gives rise to a morphism of fibered categories
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=2em, column sep=1em,text height=1.5ex, text depth=0.25ex] 
{  \Alg\left(\Qcoh\right) &        & \bd{Arr}(\Sch_k)  \\
          &\Sch_k &  \\} ;
\path[->,line width=1.0pt,font=\scriptsize]
