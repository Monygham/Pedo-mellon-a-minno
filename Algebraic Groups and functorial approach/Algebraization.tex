...
\section{Quasi-coherent $\bd{M}$-sheaves}
\noindent
...
{ \bd{M}\times_k\Spec_X\cA   &   \Spec_X\cA  \\
  \bd{M}\times_kX            &   X           \\};
\path[->,line width=1.0pt,font=\scriptsize]    
(m-1-1) edge node[above] {$ \tilde{a} $} (m-1-2)
...
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=2em, column sep=2em,text height=1.5ex, text depth=0.25ex]
{ a^*\tilde{X}    & \tilde{X}  & \pi^*\tilde{X}  & \tilde{X}     \\
...
(m-2-3) edge node[below] {$ \pi $} (m-2-4)
(m-1-3) edge node[left] {$ p' $} (m-2-3)
(m-1-4) edge node[right] {$ p $} (m-2-4);
\end{tikzpicture}
\end{center}
...
{ \bd{M}\times_k\tilde{X} & \pi^*\tilde{X} & a^*\tilde{X} & \tilde{X}  \\};
\path[->,line width=1.0pt,font=\scriptsize]
(m-1-1) edge node[auto]  {$ \cong  $} (m-1-2)
(m-1-2) edge node[auto]  {$ \tilde{\phi} $} (m-1-3)
(m-1-3) edge node[auto]  {$ a' $} (m-1-4);
\end{tikzpicture}
\end{center}
We denote it by $a_{\phi}$. Note that the square
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=2em, column sep=2em,text height=1.5ex, text depth=0.25ex] 
{ \bd{M}\times_k\tilde{X}    &   \tilde{X}    \\
  \bd{M}\times_kX            &   X           \\};
\path[->,line width=1.0pt,font=\scriptsize]    
(m-1-1) edge node[above] {$ a_{\phi} $} (m-1-2)
(m-2-1) edge node[below] {$ a $} (m-2-2)
(m-1-1) edge node[left]  {$ 1_{\bd{M}}\times_kp $} (m-2-1)
(m-1-2) edge node[right] {$ p  $} (m-2-2);
\end{tikzpicture}
\end{center}
is cartesian. Moreover, if we have a commutative triangle
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=2em, column sep=2em,text height=1.5ex, text depth=0.25ex] 
{ \tilde{X}_1    &       &  \tilde{X}_2    \\
                 &   X   &        \\};
\path[->,line width=1.0pt,font=\scriptsize]    
(m-1-1) edge node[above] {$ \tilde{f}  $} (m-1-3)
(m-1-1) edge node[below = 2pt, left] {$ p_1  $} (m-2-2)
(m-1-3) edge node[below = 2pt, right]  {$ p_2  $} (m-2-2);
\end{tikzpicture}
\end{center}
then 
Note that $\Qcoh_{\bd{M}}(X)$ admits the structure of a monoidal category such that the forgetful functor $\Qcoh_{\bd{M}}(X)\ra \Qcoh(X)$ is a strict monoidal functor. Indeed, if $(\cF_1,\phi_1)$ are $(\cF_2,\phi_2)$ quasi-coherent $\bd{M}$-sheaves, then we define their tensor product as $\bd{M}$-sheaf $\left(\cF_1\otimes_{\cO_X}\cF_2,\phi_1\otimes_{\cO_{\bd{M}\times_kX}}\phi_2\right)$. In particular, algebras with respect to this monoidal structure are quasi-coherent $\bd{M}$-sheaves $(\cA,\phi)$ such that $\cA$ is a quasi-coherent $\cO_X$-algebra and $\phi:\pi^*\cA\ra a^*\cA$ is an isomorphism of $\cO_X$-algebras.\\
\section{}
