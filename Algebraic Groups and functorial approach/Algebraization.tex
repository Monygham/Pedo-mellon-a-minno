...
\begin{definition}
Let $\bd{G}$ be a group $k$-scheme. Let $\cZ = \{Z_n\}_{n\in \mathbb{N}}$ be a formal $\bd{G}$-scheme. Then there we have the corresponding telescope of monoidal categories
\begin{center}   
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=2em,text height=1.5ex, text depth=0.25ex] 
...
\noindent
This section is devoted to certain results in formal geometry. 
...
\begin{theorem}
Let $\bd{M}$ be a Kempf monoid and $Z$ be a locally linear $\bd{M}$-scheme. Suppose that the formal $\bd{M}$-scheme $\widehat{Z}$ is locally noetherian. Then the retraction $\pi:Z\ra Z^{\bd{M}}$ is of finite type.
\end{theorem}
\begin{proof}
...
\section{Nibylandia \lstodo{Tutaj szanowni państwo kończy się uporządkowany świat i zaczyna się...}}
\noindent
...
\begin{proposition}[compatibility with identity]
...
\begin{proposition}[canonical linearization of the structure sheaf]
