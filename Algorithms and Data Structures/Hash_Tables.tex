...
\section{Analysis of hash tables with chaining for universal hashing families}
...
$$P\big(h(k_1) = h(k_2)\big) = \frac{1}{m}$$
...
\begin{example}
Let $p$ be a prime number. Consider $\cU = \ZZ_p^n$ for some $n\in \NN_+$. Fix $k$ in $\cU$. Expand
$$k = a_{n-1}(k)\cdot p^{n-1}  + ... + a_1(k) \cdot p + a_0(k)$$
where $a_0(k),a_1(k),...,a_{n-1}(k)\in \{0,1,...,p-1\}$. Then the map
$$\cU \ni k \mapsto (a_0(k),a_1(k),...,a_{n-1}(k))\in \ZZ_p^n$$
is bijective. Now for fixed $v = (v_0,...,v_n)\in \ZZ_p^n$ we define $h_v:\cU\ra \{0,1,...,p-1\}$ by formula
$$h_v(k) = \sum_{i=0}^{n-1}a_i(k)\cdot v_i\,\mathrm{mod}\,p$$   
\end{example}
