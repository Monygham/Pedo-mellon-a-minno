...
\begin{definition}
Let $\cC$ be a category and let $(s_1,t_1)$, $(s_2,t_2)$ be a directed graphs in $\cC$. Then a commutative diagram
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
{A_2 & O_2   \\
 A_1 & O_1   \\} ;
\path[->,font=\scriptsize,line width=1.0pt]
(m-1-1) edge[transform canvas={yshift =  0.5ex}] node[above] {$s_1 $} (m-1-2)
(m-1-1) edge[transform canvas={yshift = -0.5ex}] node[below] {$t_1 $} (m-1-2)
(m-2-1) edge[transform canvas={yshift =  0.5ex}] node[above] {$s_2 $} (m-2-2)
(m-2-1) edge[transform canvas={yshift = -0.5ex}] node[below] {$t_2 $} (m-2-2)
(m-2-1) edge node[left] {$f_1 $} (m-1-1)
(m-2-2) edge node[left] {$f_0 $} (m-1-2);
\end{tikzpicture}
\end{center}
is \textit{a morphism of directed graphs in $\cC$}.
\end{definition}
\noindent
The similarly a morphism of the usual directed graphs is appropriate commutative diagram of classes. It can interpreted as a morphism of directed graphs in a category $\Set_V$ for a sufficient Grothendieck universe $V$.
Next for every directed graph there exists a free category generated by it. Its construction is described {\cite[page 49, Theorem 1]{Maclane}}. Here we describe its universal property. First a free category generated by a directed graph $(s,t)$ with $s,t:A\ra O$ is a data consisting of a category $\bd{Cat}_{(s,t)}$ and a certain morphism of directed graphs which is the commutative diagram
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
{\Mor(\bd{Cat}_{(s,t)}) & \mathrm{ob}(\bd{Cat}_{(s,t)})   \\
        A         &   O\\} ;
\path[->,font=\scriptsize,line width=1.0pt]
(m-1-1) edge[transform canvas={yshift =  0.5ex}] node[above] {$\mathrm{dom} $} (m-1-2)
(m-1-1) edge[transform canvas={yshift = -0.5ex}] node[below] {$\mathrm{cod} $} (m-1-2)
(m-2-1) edge[transform canvas={yshift =  0.5ex}] node[above] {$\mathrm{dom} $} (m-2-2)
(m-2-1) edge[transform canvas={yshift = -0.5ex}] node[below] {$\mathrm{cod} $} (m-2-2)
(m-2-2) edge node[right] {$= $} (m-1-2);
\path[right hook->,font=\scriptsize,line width=1.0pt]
(m-2-1) edge node[left] {$j_A $} (m-1-1);
\end{tikzpicture}
\end{center}
Let $\cC$ be a category. Then for every commutative diagram of the form
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
...
Now suppose that and $A$ are magmas. Next we define a magma $\bd{A}_S$ and two
