...
\begin{definition}
Let $\cC$ and $\cD$ be monoidal categories. Suppose that $F:\cC\ra \cD$ is a functor, $\tau_{X,Y}:F(X\otimes Y) \ra F(X)\otimes F(Y)$ is an morphism natural in objects $X$, $Y$ of $\cC$ and $\phi:F(I)\ra I$ is an isomorphism in $\cD$. Assume that the following diagrams are commutative.
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
...
\path[->,font=\scriptsize,line width=1.0pt]
(m-1-1) edge node[above] {$\sigma_{X\otimes Y} $} (m-1-2)
(m-2-1) edge node[below] {$ \sigma_X\otimes \sigma_Y $} (m-2-2)
...
(m-1-1) edge node[left = 7pt, below = 2pt] {$ \phi^F $} (m-2-2)
(m-1-3) edge node[right = 7pt, below = 2pt] {$\phi^G $} (m-2-2);
\end{tikzpicture}
\end{center}
