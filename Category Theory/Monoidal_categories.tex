...
\end{tikzpicture}
\end{center}
is commutative. Now definition of $\tau^G$ implies (details are left to the reader) that 
...
By universal property of $\xi_{(X\otimes Y)\otimes Z)}$, we deduce that the hexagonal diagram above is commutative. Using exactly the same trick we show that triangles
\begin{center}
\begin{tikzpicture}
[description/.style={fill=white,inner sep=2pt}]
\matrix (m) [matrix of math nodes, row sep=3em, column sep=3em,text height=1.5ex, text depth=0.25ex] 
