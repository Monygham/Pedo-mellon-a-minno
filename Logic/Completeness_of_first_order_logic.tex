...
\begin{theorem}[Soundness of $\vdash$]\label{theorem:soundness_theorem}
...
\begin{theorem}[Stability under automorphisms]\label{theorem:stability_under_automorphisms}
...
Every consistent set of formulas of a first order language has a model.
\end{theorem}
\noindent
...
\begin{corollary}[Gödel's completeness theorem]\label{corollary:Gödels_completeness_theorem}
...
\begin{definition}
Let $\Gamma$ be a set of formulas of a first order language $\cL$ with identity. Let $\fU$ be a model of $\cL$ and let $s: \bd{V} \ra |\fU|$ be an interpretation. Suppose that for all $\phi$ in $\Gamma$ we have $\fU \vDash \phi[s]$. Then we say that $\fU$ together with $s$ is \textit{a model for $\Gamma$}.
\end{definition}

\begin{theorem}[Henkin’s consistency theorem for languages with identity]\label{theorem:consistency_theorem_for_languages_with_identity}
Every consistent set of formulas of a first order language with identity has a model.
\end{theorem}
\begin{proof}
Let $\cL$ be a first order language with identity and let $\Gamma$ be a consistent set of formulas of $\vdash_I$. Then by Theorem \ref{theorem:consistency_theorem} there exist a model $\fU$ of $\cL$ as a first order language (without identity) and an interpretation $s:\bd{V}\ra |\fU|$ which form a model of $\Gamma \cup I$. Now consider the set
$$\big\{s(t)\in |\fU|\,\big|\,\mbox{$t$ is a term of $\cL$}\big\}$$
Observe that this set is closed under operations $\{f^\fU\}_{f\in F}$. Thus one can restrict functions $\{f^\fU\}_{f\in F}$ and relations $\{r^{\fU}\}_{r\in R}$ to this set. This gives rise to a submodel of $\fU$. Clearly $s$ takes values in this submodel and hence this submodel together with the restriction of $s$ is a model of $\Gamma \cup I$. Thus without loss of generality we may assume that
$$|\fU| = \big\{s(t)\in |\fU|\,\big|\,\mbox{$t$ is a term of $\cL$}\big\}$$
Note that $=^\fU$ is not required to be the identity on $|\fU|$. Nevertheless $\fU$ and $s$ form a model of $I$. This means that $=^\fU$ is an equivalence relation on $|\fU|$ such that
$$(u_1,...,u_n)\in r^{\fU}\,\Leftrightarrow\,(u'_1,...,u'_n)\in r^{\fU}$$
and
$$f^{\fU}(u_1,...,u_n) =^{\fU} f^{\fU}(u'_1,...,u'_n)$$
for every $n\in \NN$, $r\in R_n$, $f\in F_n$ and elements $u_1,...,u_n,u'_1,...,u'_n \in |\fU|$ such that $u_i =^{\fU} u'_i$ for $1\leq i\leq n$. Now we define a model $\fE$ of $\cL$ as the first order language with the identity. We define $|\fE|$ as the set of equivalence classes of relation $=^{\fU}$. According to the fact that $=^{\fU}$ is compatible with relations $\{r^{\fU}\}_{r\in R}$ and functions $\{f^{\fU}\}_{f\in F}$ on $|\fU|$ we derive that $r^\fU$ for $r\in R_n$ gives rise to $r^\fE\subseteq |\fE|^n$ and each $f^{\fU}$ for $f\in F_n$ induces a function $f^\fE:|\fE|^n\ra |\fE|$. Observe that $=^\fE$ is the identity on $|\fE|$. Hence $\fE$ is a model of $\cL$ as the first order language with the identity. Denote by $q:|\fU|\twoheadrightarrow |\fE|$ the quotient map with respect to $=^{\fU}$. Then we have
$$q\big(f^{\fU}(u_1,...,u_n)\big) = f^{\fE}(q(u_1),...,q(u_n))$$
and
$$\big(q(u_1),...,q(u_n)\big)\in r^{\fE}\,\Leftrightarrow\,(u_1,...,u_n)\in r^{\fU}$$
for every $n\in \NN$, $f\in F_n$, $r\in R_n$ and $u_1,...,u_n\in |\fU|$. We claim that the previous assertion together with surjectivity of $q$ imply that for every intepretation $p:\bd{V}\ra |\fU|$ and every formula $\phi$ of $\cL$ we have
$$\fU \vDash \phi[p]\,\Leftrightarrow\,\fE\vDash\phi[q\cdot p]$$
...
$$\fU \vDash \phi[s]\,\Leftrightarrow\,\fE\vDash\phi[q\cdot s]$$
for every formula $\phi$ of $\cL$. Since $\fU$ with $s$ form a model of $\Gamma$ as the set of formulas of $\cL$ considered as the first order language, we deduce that $\fE$ with $q\cdot s$ form a model of $\Gamma$ as the set of formulas of $\cL$ considered as the first order language with identity. 
\end{proof}
...
\begin{corollary}[Gödel's completeness theorem for languages with identity]\label{corollary:Gödels_completeness_theorem_for_languages_with_identity}
...
Suppose now that $\Gamma \nvdash_I \phi$. Then $\Gamma\cup \{(\neg \phi)\}$ is consistent with respect to $\vdash_I$. Thus according to Theorem \ref{theorem:consistency_theorem_for_languages_with_identity} we deduce that there exists a model $\fU$ of $\cL$ considered as the first order language with identity and an interpretation $s:\bd{V}\ra |\fU|$ which form a model of $\Gamma \cup \{(\neg \phi)\}$. This implies that $\Gamma \nvDash_I \phi$. This completes the proof that $\Gamma \vdash_I \phi$ is equivalent with $\Gamma \vDash_I \phi$.
\end{proof}
