...
\begin{definition}
\textit{A first order language $\cL$} consists of the following data. A signature $\Sigma$ called the
signature of $\cL$ and an infinite set $\bd{V}$ called the set of variables of $\cL$.
\end{definition}
\noindent
In what follows we fix a first order language $\cL$ with signature $\Sigma$ and set of variables $\bd{V}$.

\begin{definition}
The set of all words over the alphabet consisting of
$$\{\neg, \ra, \forall, (, )\} \cup R \cup F \cup \bd{V}$$
is called the set of expressions of $\cL$. The set $\{\neg, \ra, \forall, (, )\}$ is called the set of logical constants.
\end{definition}
\noindent
...
\end{cases}$$
Next we extend $K$ to more complicated words over $F \cup V$ by
...
\begin{enumerate}[label=\emph{\textbf{\emph{(\arabic*)}}}, leftmargin=3.0em]
\item Every atomic formula of $\cL$ is a formula of $\cL$.
...
\end{definition}
\noindent
The next theorem shows the unique readability of formulas of $\cL$.
...
\item $\gamma$ is of the form $(\neg \phi)$ for some formula $\phi$ of $\cL$. Then $\beta$ is either an initial segment of $(\neg$ or a concatenation of $(\neg$ with some proper initial segment $\alpha$ of $\phi$. In the first case $K(\beta) = -1$. In the second case $K(\beta) = K(\gamma) - (K(\phi) - K(\alpha)) = 1 - (1 - K(\alpha)) = K(\alpha)$ and it suffices to show that $K(\alpha) < 1$ for proper initial segment $\alpha$ of $\phi$.
\item $\gamma$ is of the form $(\phi \ra \psi)$ for some formulas $\phi, \psi$ of $\cL$. Careful investigation shows that this option reduces to proving that $K(\alpha) < 1$ for proper initial segment $\alpha$ of $\phi$ or $\psi$.
...
\begin{proof}[Proof of the theorem]
Lemma \ref{lemma:presentation_of_formulas} implies that each formula of $\cL$ admits precisely one presentation of given type. It suffices to show uniqueness. The uniqueness part of description \textbf{(i)} follows from Lemma \ref{lemma:terminal_segment_of_a_term_is_a_concatenation_of_terms}. For \textbf{(ii)} and \textbf{(iii)} it is a consequence of Lemma \ref{lemma:no_formula_is_a_proper_initial_segment_of_any_other_formula}. For \textbf{(iv)} the uniqueness is clear.
\end{proof}
\noindent
...
\begin{enumerate}[label=\textbf{(\arabic*)}, leftmargin=3.0em]
\item A nonempty set $|\fU|$ called the universe of $\fU$ or the underlying set of $\fU$.
...
$$\fU \vDash \gamma[s_1] \,\Leftrightarrow\, \fU \vDash \gamma[s_2]$$
\end{enumerate}
\end{proof}
...
\begin{definition}
Let $\Phi$ be a tautology of classical sentential logic (with connectives $\neg, \ra$) and let $p_1,...,p_n$ be all propositional variables occurring in $\Phi$. Let $\phi_1,...,\phi_n$ be formulas of $\cL$. \textit{A tautology of $\cL$} is a formula $\phi$ of $\cL$ obtained from $\Phi$ by replacing each occurrence of $p_i$ by $\phi_i$ in $\Phi$ for $i = 1, ..., n$.
\end{definition}
\noindent
Tautologies form a basis for the first group of axioms of $\vdash$. For the second group of axioms we need the notion of variable substitutabilty, which we now define.
...
\item If $\gamma$ is of the form $\forall y \phi$ for $y \in \bd{V} \setminus \{x\}$, then $[\gamma]^x_t$ is of the form $\forall y [\gamma]^x_t$.
...
\begin{definition}
Let $\Gamma$ be a set of formulas of $\cL$ and let $\phi$ be a formula of $\cL$. \textit{A deduction of $\phi$ from $\Gamma$} is a sequence $(D_1 , ..., D_n)$ of formulas of $\cL$ such that $D_n$ is $\phi$, each $D_k$ is either an element of $\Lambda_\cL \cup \Gamma$ or there exist $i, j < k$ such that $D_j$ is of the form $(Di \ra D_k)$. If there exists a deduction of $\phi$ from $\Gamma$, then we write $\Gamma \vdash \phi$ and say that \textit{$\phi$ is deducible from $\Gamma$}.
\end{definition}
...
by assumption).
\item Suppose now that there are $i, j < k$ such that $Dj = (Di \ra D_k)$. Assume also that $\fU \vDash D_i[s]$ and $\fU \vDash D_j[s]$. Then $\fU \vDash D_i[s]$ and $$\big(\fU \vDash D_i[s] \,\Rightarrow\, \fU \vDash D_k[s]\big)$$
...
\section{Metatheorems concerning $\vdash$}
\noindent
In this section we fix a first order language $\cL$ and its deductive system $\vdash$.

\begin{proposition}[Monotonicity of $\vdash$]\label{proposition:monotonicity_of_deductive}
Let $\Gamma, \Delta$ be sets of formulas of $\cL$ and let $\phi$ be a formula of $\cL$. Suppose that $\Gamma \vdash \phi$ and $\Gamma \subseteq \Delta$. Then $\Delta \vdash \phi$.
\end{proposition}
\begin{proof}
...
Let $\Gamma$ be a set of formulas of $\cL$ and let $\phi$ be a formula of $\cL$. Suppose that $\Gamma \vdash \phi$. Then there exists finite subset $\{\phi_1,..., \phi_m\}$ of $\Gamma$ such that $\{\phi_1, ...\phi_m\} \vdash \phi$.
\end{proposition}
\begin{proof}
...
Let $\Gamma, \Delta$ be sets of formulas of $\cL$ and let $\phi$ be a formula of $\cL$. Suppose that $\Gamma \vdash \psi$ for every formula $\psi \in \Delta$ and $\Delta \vdash \phi$. Then $\Gamma \vdash \phi$.
\end{proposition}
\begin{proof}
...
Let $\Gamma$ be a set of formulas of $\cL$ and let $\phi, \psi$ be formulas of $\cL$. Then $\Gamma \cup \{\phi\} \vdash \psi$ if and only if $\Gamma \vdash (\phi \ra \psi)$.
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
Let $(D_1,...,D_n)$ be a deduction of $\phi$ from $\Gamma$. We now prove that for each $k$ there exists a deduction of $\forall xD_k$ from $\Gamma$. For this we consider the following cases.
\begin{enumerate}[label=\textbf{(\arabic*)}, leftmargin=3.0em]
...
$$\bigg(\forall xD_i , \forall x (Di \ra D_k ) , \big(\forall x (D_i \ra D_k ) \ra (\forall xD_i \ra \forall xD_k)\big) , (\forall xD_i \ra \forall xD_k) , \forall xD_k\bigg)$$
...
\begin{proposition}
Let $\cL$ be a first order language with $\bd{V}$ as the set of variables and let $\Gamma$ be a set of formulas of $\cL$. Suppose that $\fU$ is a model of $\cL$ and $s:\bd{V}\ra |\fU|$ is an interpretation of $\cL$ in $\fU$. If $\fU$ and $s$ form a model for $\Gamma$, then $\Gamma$ is consistent.
\end{proposition}
\begin{proof}
Consider a formula $\phi$ of $\cL$. We have either $\fU \nvDash \phi[s]$ or $\fU \nvDash \phi[s]$. Thus we have either $\fU \nvDash \phi[s]$ or $\fU \nvDash (\neg \phi)[s]$. From Theorem \ref{theorem:soundness_theorem} and from the fact that $\fU$ with $s$ form a model of $\Gamma$ we deduce that either $\Gamma \nvdash \phi$ or $\Gamma \nvdash (\neg \phi)$.
\end{proof}

\begin{definition}
Let $\cL$ be a first order language and let $\Gamma$ be a set of formulas of $\cL$. We say that $\Gamma$ is maximally consistent if there are no consistent sets of formulas of $\cL$ properly containing $\Gamma$.
\end{definition}
...
$$\bd{V}_{\cL} \big(\bd{Var}(D_1) \cup ... \cup \bd{Var}(D_n) \cup \bd{Var}(\phi)\big)$$
...
\end{proof}
\noindent
We prove now the following result called \textit{lemma on alphabetic variants} in literature.
...
\end{proof}
\noindent
This finishes the technical preparation. The following three lemmas are crucial and form the
conceptual part of the proof.
...
$$\Xi = \Gamma \cup \bigg\{\left(\neg[\phi_1]^{x_1}_{z_{\phi_1,x_1}}\right),...,\left(\neg[\phi_n]^{x_n}_{z_{\phi_n,x_n}}\right),\left(\neg[\phi_{n+1}]^{x_{n+1}}_{z_{\phi_{n+1},x_{n+1}}}\right)\bigg\}$$
is consistent with respect to $\vdash_{\cM}$. Write $\phi = \phi_{n+1}$ and $x = x_{n+1}$. We have that $\Xi \cup \{\left(\neg[\phi]^x_{z_{\phi,x}}\right)\}$ is inconsistent with respect to $\vdash_{\cM}$. We denote by $\perp$ any formula of $\cM$ that is a negation of a tautology of $\cM$. Then
$$\Xi \cup \{\left(\neg[\phi]^x_{z_{\phi,x}}\right)\} \vdash_{\cM} \perp$$
By Theorem \ref{theorem:deduction_theorem} we derive that
$$\Xi \vdash_{\cM} \big(\left(\neg[\phi]^x_{z_{\phi,x}}\right) \ra \perp\big)$$
Since formula
...
case.
\begin{itemize}
\item Suppose first that $\fU \nvDash \forall x\phi[s]$. Then there exists interpretation $\tilde{s}:\bd{V}_{\cL} \ra |\fU|$ and a term $t$ of $\cL$ such that $\tilde{s}(y) = s(y)$ for every variable $y \neq x$, $\tilde{s}(x) = t$ and $\fU \nvDash \phi[\tilde{s}]$. We pick formula $\alpha(\phi)$ for $\phi, x$ and $t$ as in Lemma \ref{lemma:alphabetic_variants}. That is $\phi \vdash_{\cL} \alpha(\phi)$, $\alpha(\phi) \vdash_{\cL} \phi$ and $t$ is substitutable for $x$ in $\alpha(\phi)$. Theorem \ref{theorem:soundness_theorem} implies that $\alpha(\phi) \vDash \phi$. Since
$$\fU \nvDash \phi[\tilde{s}]$$
we deduce that
